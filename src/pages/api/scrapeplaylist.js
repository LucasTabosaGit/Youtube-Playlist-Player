...
export default async function handler(req, res) {
    ...
    const { url, playlist } = req.body; // Adicione playlist à desestruturação
    ...
    try {
        ...
        const videoDetails = await page.evaluate((playlist) => { // Receba a playlist como parâmetro
            ...
            return videoElements.map(video => {
                ...
                const genre = '';
                // Use a playlist recebida como valor
                const cleanedLink = link.split('&list=')[0];

                return { name, link: cleanedLink, thumbnail, artist, duration, genre, extractedAt, playlist };
            });
        }, playlist); // Passe a playlist como argumento
